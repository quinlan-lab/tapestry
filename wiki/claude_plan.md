...
wiki/
  ...
  trio_wise_workflow/          # stubbed
    index.md
```

Step 2 (`aligned_bam_to_cpg_scores.sh`) is a pure pass-through of the
upstream pb-CpG-tools binary; called out where relevant in the
read-backed-phasing or founder-phased-methylation page rather than
given its own page.
...
### 5.6 `pedigree_wise_workflow/founder_phased_methylation/`
...
### 5.7 `pedigree_wise_workflow/all_cpg_expansion/`
...
Pending: `pedmec_phasing/` (whatshap/pedMEC), `parent_haplotype_phasing/`
...
3. **GitHub Pages vs. raw markdown.** Upstream uses raw markdown
