...
class HaplotypeAncestryPlotTests(unittest.TestCase):
    ...
    def test_bundle_is_offline_lazy_and_exposes_interactions(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("Genome-wide overview", qc_document)
            self.assertIn("Chromosome-relative outliers", qc_document)
            self.assertIn("Concordance outliers first", qc_document)
            self.assertIn("Completeness outliers first", qc_document)
            self.assertIn("Selected pair only", qc_document)
            self.assertIn("Top 50 pairs", qc_document)
            self.assertIn("Ranked ${familyName} outliers", qc_document)
            self.assertIn("Chromosomes that drive the score", qc_document)
            self.assertIn("Mismatch-excluded fraction", qc_document)
            self.assertIn("Paired CpG yield", qc_document)
            self.assertIn("Genome-wide agreement", qc_document)
            self.assertIn("robustOutliers", qc_document)
            self.assertIn("overviewRow", qc_document)
            self.assertIn("for (const row of QC.comparisons)", qc_document)
            self.assertNotIn(
                "QC.comparisons.filter(row => edgeKey(row) === edge)", qc_document
            )
            self.assertIn("rankedEdges(family).slice(0, 10)", qc_document)
            self.assertIn("drivers.scrollIntoView", qc_document)
            self.assertNotIn("heatmap.scrollIntoView", qc_document)
            self.assertIn("Click a column to inspect", qc_document)
            self.assertIn("Missing transmission comparisons", qc_document)
            self.assertIn("missing-comparison chart below", qc_document)
            self.assertNotIn("missing-comparison chart above", qc_document)
